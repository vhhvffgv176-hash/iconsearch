...
export const integrationCatalog: Record<IntegrationSlug, IntegrationConfig> = {
  ...
  'mcp-server': {
    ...
    previewContext: 'iconsearch_get_project_icons - MCP client',
    previewAction: 'Read project memory',
    outputLabel: 'Approved assignment',
    output: '{ "billing-history": "lucide/receipt-text" }',
    account: 'Free account',
    dragAndDrop: false,
    ...
    features: [
      ...
      { title: 'Audit before release', text: 'Detect missing or changed managed icons, inline SVGs, unmanaged assets, and mixed icon packages.' },
      { title: 'Bounded and revocable', text: 'Writes remain inside the validated project root, while browser sign-in creates a revocable local session.' },
    ],
